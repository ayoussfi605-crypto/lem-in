...
func main() {
	if len(os.Args) != 2 {
		return // Aw "Usage: go run . file.txt"
	}
	Parsfile(os.Args[1])
}
...
func Parsfile(filename string) {
	...
	seenCoords := make(map[string]bool) // ISLAH 1: Unique Coordinates
	...
	for _, raw := range lines {
		...
		if strings.HasPrefix(line, "##") {
			...
			if line == "##start" {
				...
				// ISLAH 2: Ignore unknown commands (like ##anything)
				continue
			}
		}
